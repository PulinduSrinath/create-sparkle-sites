    ...
    <section id="about" className="py-32 relative overflow-hidden bg-background">
      ...
      <div className="container mx-auto px-6 relative z-10">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-24">
          ...
          <h2 className="text-6xl md:text-8xl font-bold tracking-tighter text-foreground mt-8">The ZetasBuild <span className="text-primary italic">Philosophy.</span></h2>
        </motion.div>
        <div className="grid grid-cols-12 gap-8 max-w-7xl mx-auto">
          <motion.div initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} className="col-span-12 lg:col-span-8 bg-card border border-border rounded-[2.5rem] p-12 md:p-16 flex flex-col justify-between relative overflow-hidden group shadow-xl shadow-black/5">
            <div className="relative z-10">
              <h3 className="text-3xl md:text-5xl font-bold text-foreground mb-8 tracking-tight leading-tight">We don't just build websites; we craft <span className="text-primary italic">digital experiences</span> that redefine industries.</h3>
              <p className="text-muted-foreground text-xl leading-relaxed max-w-2xl font-medium">Our approach merges architectural precision with creative soul. Every line of code we write is a building block for a scalable, high-performance legacy.</p>
            </div>
            <Link to="/about#testimonials" className="mt-12 inline-block group/btn">
              <div className="h-14 px-8 rounded-full border border-border hover:border-primary/50 text-foreground font-bold flex items-center justify-center gap-3 transition-all hover:bg-primary/5 shadow-sm">
                What Our Clients Said <ArrowUpRight size={20} className="text-primary" />
              </div>
            </Link>
            <div className="absolute -bottom-20 -right-20 w-80 h-80 border border-border/30 rounded-full group-hover:scale-110 transition-transform duration-1000" />
          </motion.div>
          <motion.div initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="col-span-12 lg:col-span-4 bg-primary/5 border border-primary/10 rounded-[2.5rem] p-12 flex flex-col items-center justify-center text-center relative overflow-hidden shadow-lg shadow-primary/5">
            <div className="relative z-10 w-full">
              ...
              <div className="absolute inset-x-0 top-[4.5rem] flex flex-col items-center text-center">
                <span className="text-7xl font-bold text-foreground tracking-tighter">100%</span><span className="text-[10px] font-bold text-primary uppercase tracking-[0.4em] mt-2">Client Satisfaction</span>
              </div>
            </div>
            <p className="mt-10 text-muted-foreground font-medium italic relative z-10 text-lg">"Exceptional code, stunning design, and a partner you can trust globally."</p>
          </motion.div>
          {[
            ...
            <motion.div key={item.title} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.1 * i }} className="col-span-12 md:col-span-6 lg:col-span-4 bg-card border border-border rounded-[2.5rem] p-10 group hover:border-primary/20 hover:shadow-xl transition-all">
