...
import { Code2, Terminal, Layers, Box, Cpu, Cloud, Database, Layout, Smartphone, Palette, Globe, Server } from "lucide-react";
  ...
  return (
    <div className="flex overflow-hidden group select-none">
      <motion.div
        ...
        className="flex gap-6 pr-6 whitespace-nowrap min-w-full items-center"
      >
...
const TechCard = ({ tech }: { tech: typeof row1[0] }) => {
  return (
    <motion.div
      ...
      className="relative flex-shrink-0 w-64 bg-white/[0.03] border border-white/10 rounded-2xl p-6 transition-all duration-300 hover:bg-white/[0.07] hover:border-white/20 group cursor-default"
      style={{'--tech-color': tech.color} as any}
    >
      <div className="flex items-center gap-5 relative z-10">
        <div 
          className="w-12 h-12 rounded-xl bg-black/40 border border-white/5 flex items-center justify-center transition-transform group-hover:scale-110 shadow-inner"
      ...
      {/* Decorative Aura */}
      <div 
        className="absolute -bottom-4 -left-4 w-24 h-24 blur-[40px] opacity-0 group-hover:opacity-20 transition-opacity duration-300 pointer-events-none"
        style={{ background: tech.color }}
      />
      <div 
        className="absolute top-0 right-0 w-[1px] h-full bg-gradient-to-b from-transparent via-[var(--tech-color)] to-transparent opacity-0 group-hover:opacity-40 transition-opacity"
      />
    </motion.div>
  );
};
    ...
    <section id="tech" className="py-32 relative overflow-hidden bg-[#030712]">
      ...
      <div className="container mx-auto px-6 relative z-10 mb-20 text-center">
        <motion.div
          ...
          viewport={{ once: true }}
        >
          <span className="text-primary text-[10px] font-bold tracking-[0.6em] uppercase px-6 py-2 rounded-full border border-primary/20 bg-primary/5 mb-8 inline-block">
            Technological Core
          </span>
          <h2 className="text-6xl md:text-8xl font-bold tracking-tighter text-white">
            The <span className="gradient-text">Tech Stack</span>
          </h2>
          <p className="text-slate-400 text-xl font-medium mt-6 max-w-2xl mx-auto leading-relaxed">
            Engineered with a carefully curated ecosystem of enterprise-grade tools.
          </p>
        </motion.div>
      </div>
